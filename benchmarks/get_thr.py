...
def xattn_prefill_profile(self, query_states: torch.Tensor,
    ...
    causal=True
        ...
        out, _ = flash_attn_func(q = query_states.transpose(1,2),k = key_states.transpose(1,2),v = value_states.transpose(1,2), causal = causal)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', "--name_or_path", type=str, default="meta-llama/Llama-3.1-8B-Instruct")
    ...
    print('llama threshold')
    print(torch.tensor(llama_fuse_16).shape)
    print(torch.tensor(llama_fuse_8).shape)
    ...
    for text in tqdm(texts):
        ...
        # NOTE: each forward add one threshold to each layer
        # OOM when num is large
        if num >= 32:
            break
